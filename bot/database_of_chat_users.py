...
async def init_db_chats_users():
    async with aiosqlite.connect("database_of_chat_users.db") as db:
        await db.execute('''
...
async def check_user_exist_in_chat_in_db(chat_id:int , user_id: int):
    async with aiosqlite.connect("database_of_chat_users.db") as db:
        try:
            query = '''
...
async def add_user_id_to_db(chat_id:int, user_id:int, user_name:str):

    if await check_user_exist_in_chat_in_db(chat_id, user_id):
...
async def get_users_of_chat(chat_id:int):
    async with aiosqlite.connect("database_of_chat_users.db") as db:
        try:
            query = '''
