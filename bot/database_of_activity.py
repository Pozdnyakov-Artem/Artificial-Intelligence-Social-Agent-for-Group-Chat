...
async def get_activities_from_db(user_ids, days_range: int = 7):
    if not user_ids:
...
async def merge_overlapping_periods(df, period_start, period_end):
    if df.empty:
        return []
...
async def find_gaps_between_periods(occupied_periods,period_start,period_end):
    free_periods = []
...
async def find_common_free_time(user_ids,days_range,workday_start = 9,workday_end = 20):

    activities_df = await get_activities_from_db(user_ids, days_range)
    ...
    for day_offset in range(days_range):
        ...
        if not day_activities.empty:
            occupied_periods = await merge_overlapping_periods(day_activities, period_start, period_end)

            day_free_periods = await find_gaps_between_periods(occupied_periods, period_start, period_end)
